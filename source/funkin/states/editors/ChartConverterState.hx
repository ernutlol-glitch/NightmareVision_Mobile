...
function accept()
{
    ...
    var chartPath = importDir + "chart.json";
    var metaPath  = importDir + "meta.json";

    function hasChart() return FileSystem.exists(chartPath);
    function hasMeta() return FileSystem.exists(metaPath);
    ...
    try {
     switch (curSelection)
     {
       case 0 | 1:
        if (!hasChart() || !hasMeta())
         throw "chart.json or meta.json are missing";

       case 2:
        if (!hasChart())
         throw "chart.json is missing";
        }

     switch (curSelection)
     {
        case 0:
         var v = new FNFVSlice().fromFile(chartPath, metaPath);
         for (diff in v.diffs)
          saveFromFormat(chartPath, v, diff);

        case 1:
         var c = new FNFCodename().fromFile(chartPath, metaPath);
          saveFromFormat(chartPath, c);

        case 2:
         var p = new FNFPsych().fromFile(chartPath);
         saveFromFormat(chartPath, p);
        }
       }
       catch (e:Dynamic)
       {
        CoolUtil.showPopUp(Std.string(e), "Notice");
        }
    #else
    switch (curSelection)
    {
